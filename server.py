...
# save to history (keep only last 2 days)
now_iso = datetime.utcnow().isoformat()
HISTORY.append({"time": now_iso, "ppm": data["ppm"]})

# (optional trim) remove items older than 2 days
...
async function loadChart(){
  ...
  const co2    = j.history.map(h => h.ppm.co2);
  const nh3    = j.history.map(h => h.ppm.nh3);
...
loadChart();
setInterval(loadChart, 60_000);  // refresh chart every 60s
</script>
